...
const UsersList = ({ route }) => {
  ...
  const { whoseListIsPassed, usersList } = route.params;
  //renderSingleItem
  const renderCardItem = ({ item, index }) => {
    return (
      <UserDetailsCard item={item} index={index} navigation={navigation} />
    );
  };
  // const [modalVisibility, setModalVisibility] = useState(props.modalVisibility);
  return (
    <View style={styles.mainContainerStyle}>
      <Text style={{ fontSize: 25, marginBottom: 50, marginTop: 50 }}>
        {whoseListIsPassed === "followers"
          ? "Your followers are"
          : whoseListIsPassed === "userWhoLikedIds"
          ? "List of users liked this post"
          : "You are following"}
      </Text>
      <FlatList
        data={usersList}
