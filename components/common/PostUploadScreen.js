...
const PostUploadScreen = () => {
  ...
  const uploadPost = async (url) => {
    ...
    // if (!postData.postImageLink.includes("firebasestorage")) {
    //   ToastAndroid.show("Uploading", ToastAndroid.SHORT);
    //   navigation.goBack();
    //   return;
    // }

    // if (postData.postDescription === null) return;

    const response = await sendRequestToServer("/profile/uploadPost", postData);
    if (response.success) {
      ToastAndroid.show("Post uploaded " + response.postId, ToastAndroid.SHORT);
